...
function App() {
  ...
  //ниже состояния для закрытия и открытия окна редактирования. И для написания имени. Далее идем ниже в дивы
    ...
    <div className="bg-slate-900 text-white h-screen relative">
      <div className="flex flex-col w-full p-10">
        ...
        <div className="flex items-center justify-between bg-slate-700 rounded-xl px-4">
          <input 
          type="text"
          className="w-full py-2 rounded-xl bg-slate-700 text-white ouline-none" />
      <i onClick={addTodoHandler}>
      <PlusIcon className='icons hover:opacity-70' />
      </i>
      </div>
        <div className="mt-5 flex flex-col space-y-5 sm:space-y-0 sm:grid sm:grid-cols-2 sm:gap10 lg:grid-cols-3">
          {todos?.map((todo, index) => (
            ...
            className="cursor-pointer">
              ...
              {" "}
              {todo.status && (
                <span className="test-xs text-gray-300">(Its Ok)</span>
                )
            }
            </p>
...
//XMarkIcon это икона для закрытия плашки
